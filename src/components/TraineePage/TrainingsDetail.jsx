...
//   useEffect(() => {
//     if (chapterSubmissionStatus && !chapterSubmissionStatus.canProceed) {
//       setShowMessage(true);
...
//   //         selectedOption: answer || "unanswered", // Default to "unanswered" if no answer is provided
//   //       };
//   //     }),
//   //   };
//   // };
...
//       dispatch(setTrainingAnswers(submittedAnswers)); // Dispatch when canProceed is true
//     }
//   }, [dispatch, submittedAnswers, chapterSubmissionStatus?.canProceed]);
...
//           </button>
//           {/* {chapterSubmissionStatus && !chapterSubmissionStatus?.canProceed && <p className="text-white/90 bg-red-300 py-2 px-1 rounded-sm">You cannot proceed. Read the material and Try again!</p>} */}
//           {showMessage && (
//            <p className="text-white/90 bg-red-300 py-2 px-1 rounded-sm">
//             You cannot proceed. Read the material and Try again!
//           </p>
//           )}
//          </div>
//           {!showReadingMaterial ? (
...
const TrainingsDetail = () => {
  ...
  const { chapterSubmissionStatus, trainingChapter, chapter, loading:loadingChapter, error } = useSelector((state) => state.chapter);

  const {loading:loadingTraining} = useSelector((state) => state.training);
  ...
  // Utility: Format answers for submission, ensuring unanswered questions are marked as "unanswered"
  const formatAnswers = (answers, questions) => {
    return {
      answers: questions?.map((question, index) => {
        const answer = answers[`question${index}`];
          ...
          {!showReadingMaterial && (
            <div className="flex justify-between mt-4">
              ...
              {!isLastChapter ? (
                ...
                >
                  {/* Submit Training */}
                  {loadingTraining ? <Spinners className=""/> : 'Submit Training'}
                </button>
              )}
            </div>
          )}
