...
// import { startTrainingStart,resetError } from '../../redux/startTraining'; // import the correct action
...
import { startTrainingStart, resetError } from '../../redux/startTraining'; // Importing correct action
...
export default function TrainingCard() {
  ...
  const {
    ...
    progress,
    error: startError,
  } = useSelector((state) => state.startTraining);
  const navigate = useNavigate();

  // Fetch training details
  useEffect(() => {
    dispatch(resetTraining()); // Reset previous training state
    if (trainingId) {
      dispatch(fetchTrainingByIdStart({ id: trainingId }));
    }
  }, [dispatch, trainingId]);

  // Navigate to training details if progress exists
  useEffect(() => {
    if (progress) {
      navigate(`/Training/${trainingId}/details`);
    }
  }, [progress, navigate, trainingId]);

  // Clear passcode and reset error after showing error message
  useEffect(() => {
    if (startError) {
      const timer = setTimeout(() => {
        setPasscode('');
          ...
          <>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                value={passcode}
                onChange={handlePasscodeChange}
                className="w-full px-3 py-2 bg-gray-50 border rounded-md text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder-gray-500"
                placeholder="Enter passcode"
              />
            </div>
